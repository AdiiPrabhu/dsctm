...
class DMSTCNConfig:
    ...
    csag_mode: str = "attention"  # "attention" | "mean" (for noCSAG ablation, EXP-5.2)
...
class DMSTCN(nn.Module):
    def __init__(self, cfg: DMSTCNConfig):
        ...
        self.csag = CSAG(cfg.D, n_active, cfg.temperature) if n_active > 1 else None
        self.film = (
            FiLMAdapter(cfg.D, cfg.n_subjects, cfg.d_s, cfg.film_hidden)
            if cfg.use_film
            else None
        )
        self.head = Head(cfg.D, cfg.n_classes, cfg.head_hidden)

    def forward(self, X, subject_idx=None, return_aux: bool = False):
        # X: (B, T, F)
        ...
        Hp = self.film(fused, subject_idx) if self.film is not None else fused
        logits = self.head(Hp)
        if return_aux:
