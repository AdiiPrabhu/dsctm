...
class LSTMBaseline(nn.Module):
    ...
    def __init__(self, input_dim, n_classes, hidden=128, layers=2, bidirectional=True, head_hidden=128):
        super().__init__()
...
class TCNBaseline(nn.Module):
    ...
    def __init__(self, input_dim, n_classes, D=128, K=3, dilations=(1, 2, 4, 8), head_hidden=128):
        super().__init__()
        self.proj = nn.Linear(input_dim, D)
        self.tcn = Branch(D, K, dilations)
        self.head = Head(D, n_classes, head_hidden)

    def forward(self, X, subject_idx=None):
        h = self.proj(X).transpose(1, 2)
        return self.head(self.tcn(h).transpose(1, 2))


class TransformerBaseline(nn.Module):
    """Encoder Transformer: d_model=128, 4 heads, 2 layers, sinusoidal positions."""

    def __init__(self, input_dim, n_classes, d_model=128, nhead=4, layers=2, head_hidden=128, max_len=5000):
        super().__init__()
        self.proj = nn.Linear(input_dim, d_model)
        self.register_buffer("pos", self._sinusoid(max_len, d_model), persistent=False)
        enc = nn.TransformerEncoderLayer(
            d_model, nhead, dim_feedforward=4 * d_model, batch_first=True, activation="gelu"
        )
...
class ITransformerBaseline(nn.Module):
    ...
    def __init__(self, input_dim, n_classes, d_model=128, nhead=4, layers=2, head_hidden=128):
        super().__init__()
...
def build_baseline(name: str, input_dim: int, n_classes: int, head_hidden: int = 128) -> nn.Module:
    name = name.lower()
