...
class WindowedDataset:
    ...
    dataset: str = ""
    version: str = ""
    lengths: Optional[np.ndarray] = None  # (N,) valid timesteps before right padding
    ...
    def __post_init__(self):
        ...
        if self.lengths is None:
            self.lengths = np.full(len(self.X), self.X.shape[1], dtype=np.int64)
        else:
            self.lengths = np.asarray(self.lengths, dtype=np.int64)
            if len(self.lengths) != len(self.X):
                raise ValueError("lengths length mismatch")
            if np.any(self.lengths < 1) or np.any(self.lengths > self.X.shape[1]):
                raise ValueError("lengths must be within [1, T]")
