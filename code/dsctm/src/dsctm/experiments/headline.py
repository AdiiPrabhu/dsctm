...
def _builder(name, F, C, T):
    if name == "dmstcn":
        return (lambda n: DMSTCN(DMSTCNConfig(input_dim=F, n_classes=C, n_subjects=n)), True)
    return (lambda n: build_baseline(name, F, C, seq_len=T), False)
...
def run_studentlife_headline(ds, seeds=(0, 1, 2), cfg=None, n_splits=5,
                             ...
                             out_name="studentlife_headline.json"):
    ...
    for name in CORE_MODELS:
        build, personalize = _builder(name, ds.F, ds.n_classes, ds.T)
        seed_pooled, seed_perfold = [], []
        for s in seeds:
            run_cfg = {**cfg, "model": name, "input_dim": ds.F,
                       "n_classes": ds.n_classes}
            r = headline_cv(
                build, ds, folds, cfg, dev, seed=s, personalize=personalize,
                on_fold_complete=lambda fold, fit, s=s, run_cfg=run_cfg, name=name:
                    write_completed_fit(
                        experiment_id="EXP-4.1", condition=name, dataset=ds,
    ...
    out = {"experiment": "EXP-4.1", "dataset": "studentlife",
           ...
           "data_version": ds.version, "data_hash": ds.data_version_hash(),
...
def run_daic_headline(ds, manifest, seeds=(0, 1, 2, 3, 4), cfg=None,
    ...
    for name in CORE_MODELS:
        ...
        results[name] = {
            ...
            "per_seed_test_metrics": test_full,
            "per_seed_test_probabilities": [p.tolist() for p in probs_s],
        }
